  ...
  trait List[+T] {
    ...
    def mkString(sep: String): String = {
      ...
      def recursiveConcat(list: List[T], concated: String): String = list match {
        case List.::(head, tail) => recursiveConcat(tail, head.toString + sep + concated)
        case List.Nil            => concated
      }
      recursiveConcat(this, "")
    }
    ...
    def filter[B](f: T => Boolean): List[T] = {
      ...
      def recursiveFilter(initialList: List[T], filteredList: List[T]): List[T] =
        initialList match {
          ...
          case List.::(head, tail) if !f(head) => recursiveFilter(tail, filteredList)
          case List.Nil                        => filteredList
        }
      recursiveFilter(this, List.Nil)
    }
  }
